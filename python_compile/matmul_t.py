    ...
    for i in range(0,2048):
        for j in range(0,2048):
            for k in range(0,2048):
                Y[i][j] = Y[i][j] + A[i][k]*BT[j][k];

def py_matmul(A,BT,Y):
    for i in range(0,2048):
        for j in range(0,2048):
            for k in range(0,2048):
                Y[i][j] = Y[i][j] + A[i][k]*BT[j][k];
                ...
                for i in range(0,256):
                    for j in range(0,256):
                        for k in range(0,256):
                            Y[i+ii*256][j+jj*256] = Y[i+ii*256][j+jj*256] + A[i+ii*256][k+kk*256]*BT[j+jj*256][k+kk*256];

A = 2048.0 * np.array(np.random.rand(2048,2048), dtype=np.float32);
B = 2048.0 * np.array(np.random.rand(2048,2048), dtype=np.float32);
BT = np.zeros((2048,2048), dtype=np.float32);
