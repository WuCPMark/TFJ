...
class For(Statement):
    'For loop'
    def __init__(self, indvar, niter, body, orelse):
        self.parameters = [indvar, niter, body, orelse] 
    
    def __str__(self):
        indvar = str(self.parameters[0])
        lb = self.parameters[1].arguments()[0]
        ub = self.parameters[1].arguments()[1]
        body = _indent(strlist(self.parameters[2], sep='\n', form=str))
        orelse = _indent(strlist(self.parameters[3], sep='\n', form=str))
        return '<ForStmt>\n<LoopHeader>\n<Induction>%s</Induction>\n<Start>%s</Start>\n<Stop>%s</Stop>\n</LoopHeader>\n<LoopBody>\n%s\n</LoopBody>\n</ForStmt>\n' % (indvar,lb, ub,body) 

    def indvar(self):    return self.parameters[0]
    def niter(self):     return self.parameters[1]
    def lbound(self):   
        return self.parameters[1].arguments()[0]
    def ubound(self):    
        return self.parameters[1].arguments()[1]
    def body(self):      return self.parameters[2]
    def orelse(self):    return self.parameters[3]
